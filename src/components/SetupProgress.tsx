...
export function SetupProgress({
  ...
  return <Card className="bg-gradient-to-br from-primary/5 to-transparent border-primary/20 shadow-sm">
      <CardHeader className="pb-2 p-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="icon" onClick={onDismiss} className="h-6 w-6 text-muted-foreground hover:text-foreground">
            <X className="h-3 w-3" />
          </Button>
        </div>
      ...
      <CardContent className="space-y-1.5 p-4 pt-0">
        {setupSteps.map((step, index) => <div key={step.id}>
            <div className={`flex items-center justify-between p-2 rounded-lg transition-colors ${step.completed ? 'bg-success/10 border border-success/20' : step.active ? 'bg-muted/30 hover:bg-muted/50' : 'bg-muted/10 opacity-60'}`}>
              <div className="flex items-center gap-2">
                
                <div>
                  <h3 className={`text-xs font-medium ${step.completed ? 'text-success' : step.active ? 'text-foreground' : 'text-muted-foreground/50'}`}>
                    {step.title}
                  </h3>
                  <p className={`text-xs ${step.active ? 'text-muted-foreground' : 'text-muted-foreground/50'}`}>
                    {step.description}
                  </p>
                </div>
              </div>
              
              {!step.completed && step.action && <Button variant="outline" size="sm" onClick={() => onStepClick(step.id)} disabled={!step.active} className={`ml-2 flex-shrink-0 h-6 text-xs ${!step.active ? 'opacity-50 cursor-not-allowed' : ''}`}>
                  {step.action}
                </Button>}
            </div>
          </div>)}
