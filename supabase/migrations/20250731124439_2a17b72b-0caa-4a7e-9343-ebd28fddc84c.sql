-- Fix employees_status_check constraint to include all current and new valid statuses
-- First drop the existing constraint
ALTER TABLE public.employees DROP CONSTRAINT IF EXISTS employees_status_check;

-- Update any 'Exit' status to 'Inactive' to match our valid statuses
UPDATE public.employees SET status = 'Inactive' WHERE status = 'Exit';

-- Add the updated constraint that includes all valid statuses
