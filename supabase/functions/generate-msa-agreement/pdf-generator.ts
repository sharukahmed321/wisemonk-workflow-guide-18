...
export async function generateAgreementPDF(msaData: any, templateDocId?: string): Promise<Uint8Array> {
  ...
  try {
    ...
    if (diagnosticResult.status === 'success') {
      ...
      try {
        const pdfBuffer = await generateMSAWithSharedDrive(finalTemplateId, placeholders, msaData);
