...
const createZohoSignRequest = async (accessToken, pdfUrl, fileName, userProfile, organization)=>{
  ...
  let bodyParts = [];
  // Add JSON data part
  const jsonPart = new TextEncoder().encode(`--${boundary}\r\n` + 'Content-Disposition: form-data; name="data"\r\n' + 'Content-Type: application/json\r\n\r\n' + JSON.stringify(requestData) + '\r\n');
  bodyParts.push(jsonPart);
  // Add file part header
  const fileHeaderPart = new TextEncoder().encode(`--${boundary}\r\n` + `Content-Disposition: form-data; name="file"; filename="${fileName}"\r\n` + 'Content-Type: application/pdf\r\n\r\n');
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: msaDoc, error: fetchError } = await supabase.from('msa_documents').select('*').eq('id', msaDocumentId).single();
    ...
    const { data: userProfile, error: profileError } = await supabase.from('profiles').select(`
        *,
        organizations (*)
      `).eq('user_id', msaDoc.user_id).single();
    ...
    await supabase.from('msa_documents').update({
    ...
    const { requestId, documentId } = await createZohoSignRequest(accessToken, pdfUrl, fileName, userProfile, userProfile.organizations);
    ...
    const { error: updateError } = await supabase.from('msa_documents').update({
